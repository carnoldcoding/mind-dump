...
- `src/styles/` — extra CSS beyond Tailwind utilities (`animations.css`, `custom.css`). `animations.css` no longer holds the motion primitives: they are GSAP timelines now ([ADR-0007](./adr/0007-gsap-timelines-own-motion.md)), and what remains here is the ambient infinite loops — the diagonal background lines and the loader spin — plus the `motion-off` rule that suppresses hover/focus transitions. Read [`docs/motion.md`](./motion.md) before touching anything that animates; it says which of the three mechanisms owns what.
- `src/utils/animations.ts` — the single motion seam. Three inputs, in precedence order: the runtime override (the `Ctrl+Alt+M` dev chord), then `prefers-reduced-motion`, then `VITE_DISABLE_ANIMATIONS` (set in `.env.local`, build-time, restart the dev server). When motion is off, a timeline is built as normal and set to `progress(1)` — the same declaration, resolved instantly — and the seam puts `motion-off` on the document element to suppress Response transitions. The boot sequence is a timeline like any other now, though whether the seam should silence it is still open; the ambient background and the loading spinner are not gated. See [`docs/motion.md`](./motion.md).
