...
- `src/styles/` — extra CSS beyond Tailwind utilities (`animations.css`, `custom.css`). `animations.css` holds the five motion primitives — Wipe, Domino, Growth, Fade, plus the boot sequence's own keyframes. Read [`docs/motion.md`](./motion.md) before touching any of them; it says which primitive owns what. (`nier-enter` was the old panel/modal entrance and no longer exists — it was a translate-and-fade, replaced by the `clip-path` Wipe.)
- `src/utils/animations.ts` — the single motion seam. Three inputs, in precedence order: the runtime override (the `Ctrl+Alt+M` dev chord), then `prefers-reduced-motion`, then `VITE_DISABLE_ANIMATIONS` (set in `.env.local`, build-time, restart the dev server). When motion is off, panel-reveal, title decode and modal enter/backdrop resolve instantly instead of animating. Still does not affect the boot sequence, `Navigation`'s entrance, the ambient background, or the loading spinner. See [`docs/motion.md`](./motion.md).
