...
- [`docs/branching.md`](./docs/branching.md) — the git workflow, in full: branches, releases, hotfixes, versioning, and what an agent may merge. Read it rather than assuming; it was revised on 2026-08-08 and the same model governs the backend repo.
...
- Every change, including small ones, goes through a branch + PR off **`dev`**. The one case that branches off `main` is a `hotfix/*`, for something already released. Everything else about branching, releasing and merging lives in [`docs/branching.md`](./docs/branching.md) — this file deliberately does not restate it, because it used to and the two drifted.
