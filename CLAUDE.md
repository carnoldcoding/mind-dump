...
- The git workflow — branches, releases, hotfixes, versioning, and what an agent may merge — lives in the global `~/.claude/CLAUDE.md` "Branching strategy" section, the single source of truth for every repo. The same model governs the backend repo.
...
- Every change goes through a branch + PR off **`dev`**. The one case that branches off `main` is a `hotfix/*`, for something already released. Everything else about branching, releasing and merging lives in the global `~/.claude/CLAUDE.md` "Branching strategy" section — this file deliberately does not restate it.
