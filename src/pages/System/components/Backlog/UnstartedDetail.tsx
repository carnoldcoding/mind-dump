    ...
    return (
        ...
        >
            <div className="flex items-start justify-between gap-2">
                <span className="text-eyebrow uppercase tracking-widest text-nier-text-dark/50">Selected</span>
                <button
                    type="button"
                    onClick={onClose}
                    aria-label="Close detail"
                    className="text-label leading-none cursor-pointer hover:text-nier-dark transition-colors duration-150"
                >✕</button>
            </div>
            ...
            <h4 className="text-body uppercase tracking-wide text-nier-text-dark leading-tight">
                {review.title}
            </h4>

            <p className="text-eyebrow uppercase tracking-wide text-nier-text-dark/50">
                {review.type}
                {review.release_date && ` · ${review.release_date.slice(0, 4)}`}
            </p>

            {review.creator && (
                <p className="text-eyebrow uppercase tracking-wide text-nier-text-dark/60 truncate">
                    {review.creator}
                </p>
            )}

            {review.genres && review.genres.length > 0 && (
                <p className="text-eyebrow uppercase tracking-wide text-nier-text-dark/40">
                    {review.genres.join(' · ')}
                </p>
            )}
            ...
            {written.length > 0 && (
                <p className="text-label text-nier-text-dark/60" aria-label="Sections written">
            ...
            {typeof review.description === 'string' && review.description && (
                <p className="text-label text-nier-text-dark/70 line-clamp-6">{review.description}</p>
            )}
            ...
            <div className="mt-auto flex flex-col gap-px pt-2" onMouseLeave={() => setConfirming(false)}>
                <button
                    ...
                    className="text-eyebrow uppercase tracking-widest py-1.5 bg-nier-dark text-nier-text-light hover:bg-nier-text-dark cursor-pointer transition-colors duration-150"
                >Start</button>
                <div className="flex gap-px">
                    ...
                    <button
                        ...
                        className={`flex-1 text-eyebrow uppercase tracking-widest py-1.5 cursor-pointer transition-colors duration-150 ${
                            confirming
