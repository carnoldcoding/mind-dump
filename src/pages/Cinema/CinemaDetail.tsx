...
const CinemaDetail = () => {
    ...
    const [data, setData] = useState<CinemaPost | null>(null);

    const navigate = useNavigate();

    const handleClose = () => navigate('/cinema');
    ...
    const renderContent = () => {
        if (loading) return <div>Loading cinema...</div>;
        if (error) return <div>Error: {error}</div>;
        if (!data) return <div>Cinema not found</div>; 
        ...
        return (
            <article className="md:w-5xl md:h-134 bg-nier-100 mt-5 relative">
                ...
                <div className="py-4 pb-8 md:p-4 flex-col flex md:flex-row">
                    ...
                    <div className="md:ml-0 ml-5 absolute bottom-0 text-nier-text-dark text-sm md:text-base italic">{data.release_date} - {data.director}</div>
