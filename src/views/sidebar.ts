...
const createOptions = (options : string[]) => {
    ...
    options.forEach(option => {
        optionsTemplate += `<p class="p-2 text-white w-full hover:bg-slate-600 cursor-pointer select-none" data-filter=${option}>${option}</p>`;
    })
    return optionsTemplate;
}

export const createSidebar = () => {
    const options = ["action", "rpg", "stuff", "third-person", "first-person", "cool"]
    return `
        <div>
        ...
        <aside class="fixed right-0 p-3 top-0 h-full w-2xs overflow-hidden bg-black/30 backdrop-blur-md z-11 duration-75 border-l border-slate-200/20 shadow-md" id="game-list-controls">
          ...
          <div class="flex flex-col gap-3">
            ...
            <div>
              <div class="rounded-sm text-white w-full p-2 bg-slate-700 relative flex justify-between items-center cursor-pointer">
            ...
            <button class="bg-green-300 hover:bg-green-200 py-2 px-5 rounded-md cursor-pointer capitalize grow">
              apply
            </button>
            <button class="bg-red-300 hover:bg-red-200 cursor-pointer py-2 px-5 rounded-md capitalize grow">
              reset
            </button>
          </div>
        </aside>
