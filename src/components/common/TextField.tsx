...
export const TextField = ({ label, value, onChange, disabled, altBg, type, autofillData }: TextFieldProps) => {
  ...
  return (
    <div className="border border-nier-150 flex h-12 relative w-full">
      ...
      />
      <label htmlFor={id} className={`absolute left-4 top-3 text-gray-500 pointer-events-none transition-all peer-focus:top-[-10px] peer-focus:left-2 peer-focus:text-body peer-focus:px-1 peer-[:not(:placeholder-shown)]:top-[-10px] peer-[:not(:placeholder-shown)]:left-2 peer-[:not(:placeholder-shown)]:text-body peer-[:not(:placeholder-shown)]:px-1 ${
        altBg
